function bidsInitVista(projectDir, subject, session, tasks,... % runnums,
                       dataFolder , analysisDir, anatomyDir)
    % function bidsInitVista(projectDir, subject, [session], [tasks], [runnums], ...
    ...
    %                            fullfile(fileparts(projectDir),
    %                            'Analyses', <subj>, <session>)    
    %     anatomyDir:       path to directory where vistasoft anatomy files are contained
    %                           default: /Volumes/server/Projects/Anatomy
    % Example:
    %
